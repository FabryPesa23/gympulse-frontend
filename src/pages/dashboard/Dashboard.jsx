...
function Dashboard() {
  ...
  return (
    <Container className="mt-4 mb-5">
      ...
      {/* DESCRIZIONE APP */}
      <div className="mb-4 p-4" style={{ ...cardStyle, borderRadius: '16px' }}>
        <Row className="align-items-center">
          <Col xs={12} md={7} className="mb-3 mb-md-0">
            <h4 style={{ color: '#ff6b00', marginBottom: '12px' }}>
              Il tuo allenamento, <span style={{ color: '#ffffff' }}>smart</span> 🏋️
            </h4>
          ...
          <Col xs={12} md={5}>
            <Row>
              {[
                ...
                { emoji: '⏳', title: 'Lista d\'attesa', text: 'Entra automaticamente se si libera un posto' },
              ].map(({ emoji, title, text }) => (
                <Col key={title} xs={6} className="mb-3">
                  <div style={{
                    ...
                    <p style={{ color: '#ffffff', fontSize: '0.8rem', fontWeight: '600', marginBottom: '2px' }}>{title}</p>
                    <p style={{ color: '#aaaaaa', fontSize: '0.75rem', marginBottom: 0 }}>{text}</p>
                  </div>
                </Col>
              ))}
            </Row>
