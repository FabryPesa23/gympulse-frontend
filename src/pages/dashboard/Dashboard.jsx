  ...
  return (
    <Container className="mt-4 mb-5">
        ...
        <Row className="align-items-center">
          <Col>
            <h2 style={{ color: '#ffffff', fontWeight: '700' }}>
              Bentornato, <span style={{ color: '#ff6b00' }}>{user?.firstName}</span>! 💪
            </h2>
      ...
      <Row className="mb-4">
        ...
        <Col xs={6} md={3} className="mb-3">
          <Card style={cardStyle} className="text-center p-3">
            <h2 style={{ color: '#ff6b00', fontWeight: '700' }}>{bookings.filter(b => b.status === 'CANCELLED').length}</h2>
                ...
                zones.map(zone => (
                  <div key={zone.id} className="mb-3">
                    <div className="d-flex justify-content-between mb-1">
                      ...
                      <span style={{ fontSize: '0.85rem' }}>{getStatusLabel(zone.occupancyStatus)}</span>
                    </div>
                    <ProgressBar
