...
function CourseDetail() {
  ...
  const getNextDateForDay = (dayOfWeek) => {
    ...
    const slotIndex = days.indexOf(dayOfWeek);
  ...
  const fetchSlots = async () => {
    try {
      const response = await fetch(
        `http://localhost:8080/api/timeslots/course/${id}`,
      );
      const data = await response.json();
      ...
      const slotsWithDates = await Promise.all(
        data.map(async (slot) => {
          ...
          const res = await fetch(
            `http://localhost:8080/api/timeslots/course/${id}?date=${nextDate}`,
          );
          const updatedSlots = await res.json();
          const updated = updatedSlots.find(
            (s) => s.id === slot.id && s.dayOfWeek === slot.dayOfWeek,
          );
          return updated ? { ...updated, dayOfWeek: slot.dayOfWeek } : slot;
        }),
      );
      ...
      {course && (
        ...
        >
          <Card.Body>
            ...
            <p style={{ color: "#aaaaaa" }}>
              🏷️{" "}
              <span style={{ color: "#ffffff" }}>{course.category?.name}</span>
            </p>
          </Card.Body>
        </Card>
      )}
            ...
            >
              <Card.Body>
                ...
                <Button
                  ...
                  style={{
                    backgroundColor:
                      slot.availableSlots === 0 ? "transparent" : "#ff6b00",
                    border:
                      slot.availableSlots === 0 ? "1px solid #ffc107" : "none",
